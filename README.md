This is an introductory tutorial on Docker containers. By the end of this article you will get the idea on how to use Docker on your local machine. Along with Python we are going to run Nginx and Redis containers. Those examples assume that you are familiar with the basic concepts of those technologies. There will be lots of shell examples, so go head and open the terminal.

# Table of contents

* [What is Docker?](#what-is-docker)
* [What is the difference from virtualization?](#what-is-the-difference-from-virtualization)
...
* [Best practices of creating images](#best-practices-of-creating-images)
...
[Docker](https://www.docker.com/) is an open source tool that automates the deployment of the application inside software container. 
...
With the introduction of containers, bricks can be put over a glass, and chemicals can be stored next to food. Cargo of the various size can be put inside a standardized container that can be loaded/unloaded by the same vehicle.

Let’s go back to **containers in software development**.

When you develop an application, you need to provide your code alongside with all possible dependencies like libraries, web server, databases, etc. You may end up in a situation when the application is working on your computer but won’t even start on staging server, dev or a QA’s machine.
...
There is no need to install 3rd parties like PostgreSQL, Redis, Elasticsearch into system -- you can run it in containers. Docker also give you an ability to run different versions of same application simultaneously. For example you need to make some manual data migration from older version of Postgres to newer version. You can have such situation in microservice architecture, when you want to create new microservice with new version of 3rd party.

It could be quite complex to keep two different versions of the same app on one host OS. In this case Docker containers could be a perfect solution -- you receive an isolated environments for your applications and 3rd parties.

### Handy application encapsulation

You can deliver your application in one piece. Most programming languages, frameworks and all operation systems have its own packaging managers. And even if your application could be packed with its native package manager it could be hard to make a port for another system.

Docker gives you unified image format to distribute you applications across different host systems and cloud services. You can deliver your application in one piece with all required dependencies (included in image) ready to run.

### Same behaviour on local machine / dev / staging / production servers

Docker could not guarantee 100% dev / staging / production parity, because there could be a human factor. But it reduce almost to zero a probability of error caused by different version of operation system, system-dependencies, etc.

With right approach of building Docker images you application will use the same base image with the same version of OS and required dependencies.

### Easy and clear monitoring

Out of the box you have a unified way to read log files from all running containers. You don't need to remember all specific paths where your app and its dependencies store log files and write some custom hooks to handle it.

You can integrate [external logging driver](https://docs.docker.com/config/containers/logging/configure/#supported-logging-drivers) and monitor your app log files in one place.

### Easy to scale

Correctly wrapped application will cover most of [Twelve Factors](https://12factor.net/). By design Docker forces you follow its core principles such as: configuration over environment variables, communication over TCP/UDP ports, etc. And if you’ve done your application right it will be ready for scaling not only in Docker.
...
You can check the installation instructions [here](https://docs.docker.com/install/).
...
* **Container** -- running instance that encapsulates required software. Containers are always created from images. Container can expose ports and volumes to interact with other containers or/and outer world. Container can be easily killed / removed and re-created again in a very short time. Containers don't keep state.

* **Image** -- basic element for every container. When you create an image every step is cached and can be reused ([Copy On Write model](https://en.wikipedia.org/wiki/Copy-on-write)). Depending on the image it can take some time to build it. Containers, on the other hand can be started from images right away.
...
**Warning:** This command looks quite heavy, but it's just an example to explain volumes and env variables. In real life in 99% of cases you won't start Docker containers manually -- you'll use orchestration services (we'll cover [docker-compose](https://docs.docker.com/compose/overview/) in [example #4](#example-4-connection-between-containers)) or probably write a custom script to do it.
...
Now you can check this [url](http://127.0.0.1:8080/) in your web browser.
...
A lot of Docker images (versions of images) are created on the top of [Alpine Linux](https://alpinelinux.org/) -- this is a lightweight distro that allows you to reduce the overall size of Docker images.

I recommend using images based on Alpine for third-party services, such as Redis, Postgres, etc. For your app images I recommend to use images based on [buildpack](https://hub.docker.com/_/buildpack-deps/) -- it will be easy to debug inside container and you'll have a lot of pre-installed system-wide requirements.

Only you decide which base image to use, but you can get the maximum benefit by using one basic image for all images, because in this case cache will be used more effectively.

# Example 4: Connection between containers

[**Docker compose**](https://docs.docker.com/compose/overview/) -- is an CLI utility to connect containers with each other.
...
Current example will increment view counter in Redis. Open the [following url](http://127.0.0.1:5000/) in your web browser and check it.

Using docker-compose is a topic for a separate tutorial. To get started you can play with some images from Docker Hub or if you want to create your own images -- follow the best practices listed above. The only thing I can add in terms of using docker-compose: **always give explicit names to your volumes** in docker-compose.yml (if image has volumes). This simple rule will save you from an issue in the future when you'll be inspecting your volumes.
...
To summarize this tutorial, alongside with IDE and Git, Docker has become one of the must-have developer tools. It's a production ready tool with rich and mature infrastructure.

Docker can be used on all types of projects regardless of size and complexity. At the beginning you can start with [compose](https://docs.docker.com/compose/overview/) and [Swarm](https://docs.docker.com/engine/swarm/). And when the project grows you can migrate to cloud services like [Amazon Container Services](https://aws.amazon.com/containers/) or [Kubernetes](https://kubernetes.io/).

Like standart containers used in cargo transportation, wrapping your code in Docker containers will help to build a faster and efficient CI/CD processes. This is not just another technological trend promoted by a bunch of geeks -- it's a new paradigm that is already being used in the architecture of large companies like [PayPal](https://blog.docker.com/2017/12/containers-at-paypal/), [Visa](https://blog.docker.com/2017/04/visa-inc-gains-speed-operational-efficiency-docker-enterprise-edition/), [Swisscom](https://www.docker.com/customers/swisscom-goes-400vms-20vms-docker), [General Electric](https://www.docker.com/customers/ge-uses-docker-enable-self-service-their-developers), [Splink](https://www.docker.com/customers/docker-datacenter-delivers-splunks-house-demos), etc.
